...
```{r project.setup, message=FALSE, error=FALSE, warning=FALSE, results='hide'}

#Import libraries
library(tidyverse);library(lubridate);library(here);library(knitr);library(ggthemes)
...
```{r month.setup, message=FALSE, echo=FALSE, warning=FALSE, results='hide'}
#checking to see if month is a factor and converting to factor
class(epa_data$Month)
unique(epa_data$Month)
...
```{r ozone_2018, echo=FALSE, warning=FALSE, fig.align='left', fig.height=4, out.width='85%', fig.cap="2018 Ozone Levels by Month"}

ozone_2018 <- epa_data %>% 
  filter(year(Date) == 2018)

ggplot(ozone_2018, 
       aes(x = Month_f, y = Ozone)) +
  geom_boxplot() +
...
```{r ozone_2019, echo=FALSE, warning=FALSE, fig.align='left', fig.height=4, out.width='85%', fig.cap="2019 Ozone Levels by Month"}

ozone_2019 <- epa_data %>% 
  filter(year(Date) == 2019)

# Create boxplot for Ozone levels by month for 2019
ggplot(ozone_2019, aes(x = Month_f, y = Ozone)) +
  geom_boxplot() +
  labs(x = "Month", y = "Ozone Levels",) +
...
## Git Repository
https://github.com/YeKhaungOo/EDA_Spring2024
